...
def inverse_lbs(
    ...
    ''' Inverse Linear Blend Skinning to obtain v_template from verts.

        Parameters
        ----------
        verts : torch.tensor BxVx3
            The vertices of the deformed mesh
        betas : torch.tensor BxNB
            The tensor of shape parameters
        pose : torch.tensor Bx(J + 1) * 3
            The pose parameters in axis-angle format
        ...
        -------
        v_template: torch.tensor BxVx3
            The template mesh before applying shape and pose displacements.
    '''
    if verts.dim() == 2:
        verts = verts[None]
    ...
    # Calculate the global joint location and transformation matrices
    J_transformed, A = batch_rigid_transform(rot_mats, J, parents, dtype=dtype)

    # Calculate the skinning transformation
    W = lbs_weights.unsqueeze(dim=0).expand([batch_size, -1, -1])
    num_joints = J_regressor.shape[0]
    T = torch.matmul(W, A.view(batch_size, num_joints, 16)) \
        .view(batch_size, -1, 4, 4)
    if is_offset:
        # From verts to v_posed
        v_homo = torch.cat([verts, torch.zeros_like(verts[:, :, :1])], dim=2)
        v_posed_homo = torch.matmul(T.inverse(), v_homo.unsqueeze(-1)).squeeze(-1)
        v_posed = v_posed_homo[:, :, :3]
        return v_posed
    else:
        # From verts to v_posed
        v_homo = torch.cat([verts, torch.ones_like(verts[:, :, :1])], dim=2)
        v_posed_homo = torch.matmul(T.inverse(), v_homo.unsqueeze(-1)).squeeze(-1)
        v_posed = v_posed_homo[:, :, :3]

        # From v_posed to v_shaped
        v_shaped = v_posed - pose_offsets

        # From v_shaped to v_template
        v_template = v_shaped - shape_offsets
