...
from .lbs import (
    lbs, vertices2landmarks, find_dynamic_lmk_idx_and_bcoords, blend_shapes, inverse_lbs)
...
class SMPLLayer(SMPL):
    ...
    def canonnicalize(self,vertices,
        betas: Optional[Tensor] = None,
        body_pose: Optional[Tensor] = None,
        global_orient: Optional[Tensor] = None,
        transl: Optional[Tensor] = None,
        ...
        **kwargs
        ...
        if not is_offset:
            if transl is not None:
                vertices -= transl
        v_template_re = inverse_lbs(vertices, betas, full_pose, self.v_template,
                             self.shapedirs, self.posedirs,
                             self.J_regressor, self.parents,
                             self.lbs_weights, is_offset=is_offset,
                             pose2rot=False)


        return v_template_re
