...
def load_model(mesh_path, device=device):
    ...
    # vis_utils.visualize_mesh(verts, faces_idx)
    faces_t = faces.textures_idx.to(device)
    faces_n = faces.normals_idx.to(device)
    verts_uvs = aux[1].to(device)
    return verts, faces, faces_idx, verts_uvs, faces_t, faces_n, device
...
def diagonal_dot(a, b):
    return torch.matmul(a * b, torch.ones(a.shape[1], device=a.device))
...
def points_to_barycentric(triangles, points):
    ...
    barycentric = torch.zeros(len(triangles), 3, device=points.device)
