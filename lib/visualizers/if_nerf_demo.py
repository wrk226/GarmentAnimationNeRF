...
class Visualizer:
    def __init__(self):
        ...
        print(colored('the render results are saved at {}'.format(data_dir),
                      'yellow'))
    ...
    def visualize(self, output, batch):
        ...
        if cfg.shape_control[0]:
            for pc, val in cfg.shape_control:
                img_root += '_pc{}={}'.format(pc, val)

        # clothes change
        if cfg.upper_body:
            img_root += '_upper={}'.format(cfg.upper_body)
        if cfg.lower_body:
            img_root += '_lower={}'.format(cfg.lower_body)
