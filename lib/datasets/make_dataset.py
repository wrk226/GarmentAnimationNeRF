...
def _dataset_factory(is_train):
    ...
    dataset = importlib.import_module(module, package=path).Dataset(**args)
...
def make_data_loader(cfg, is_train=True, is_distributed=False, max_iter=-1):
    ...
    dataset = make_dataset(None, None, None, is_train)
    sampler = make_data_sampler(dataset, shuffle, is_distributed, None)
