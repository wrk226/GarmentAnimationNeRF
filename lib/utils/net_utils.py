...
def demask(x, mask):
    x_all = torch.zeros(*mask.shape, x.shape[-1], device=x.device)
    x_all[mask] = x
    return x_all
...
def load_model(net,
    ...
    pths = [
        ...
        if '.pth'in pth and pth != 'latest.pth'
    ]
    ...
    if cfg.sdf_input.prev_deform:
        net.register_dynamic_buffers(pretrained_model['net'])
...
def save_model(net, optim, scheduler, recorder, model_dir, epoch, last=False):
    ...
    # import shutil
    # shutil.rmtree(os.path.join(model_dir, '{}.pth'.format(min(pths))))
...
def filter_sdf_feature(inputs, blocked=False):
    if blocked:
        wpts = inputs[:, :3]
        wproj = inputs[:, 3:3+3*5]
        h = inputs[:, 3+3*5:]
    else:
        wpts = inputs[:, :3]
        wproj = inputs[:, 3:6]
...
def filter_var_feature(inputs):
    wpts = inputs[:, :3]
    wproj = inputs[:, 3:6]
    h = inputs[:, 6:]
    result = []
    if cfg.var_input.wpts:
        result.append(wpts)
    if cfg.var_input.wproj:
        result.append(wproj)
    if cfg.var_input.h:
        result.append(h)
    result = torch.cat(result, dim=-1)
...
def filter_color_feature(inputs):
    wpts = inputs[:, :3]
    wproj = inputs[:, 3:6]
    h = inputs[:, 6:]
    result = []
    if cfg.color_input.wpts:
        result.append(wpts)
    if cfg.color_input.wproj:
        result.append(wproj)
    if cfg.color_input.h:
        result.append(h)
    result = torch.cat(result, dim=-1)
    return result

def filter_color_dir_feature(inputs):
    wdir = inputs[:, :3]
    ldir = inputs[:, 3:]
    result = []
    if cfg.color_input.wdir:
        result.append(wdir)
    if cfg.color_input.ldir:
        result.append(ldir)
    result = torch.cat(result, dim=-1)
    return result